...
void loop(){
  ...
  client.loop();
    ...
    short estado = digitalRead(digital_1);
    short sirenas = digitalRead(digital_2);
    short mov = digitalRead(digital_3);
    short aberturas = digitalRead(digital_4);

    if(sirenas == 0){
      delay(1000);
    }

    sirenas = digitalRead(digital_2);

    if(digitalRead(digital_2) == 0){
      sirenas = 1;
    }else{
      sirenas = 0;
    }
    ...
    char topic[50];
		...
		String topic_aux = "alarma/values/" + serie_device;
    String to_send = String(estado) + "," + String(sirenas) + "," + String(mov) + "," + String(aberturas);
...
void callback(char* topic, byte* payload, unsigned int length){
  ...
  if (str_topic == "alarma/command/" + serie_device){

    if ( incoming == "activar" || incoming == "desactivar") {
      while(cont1<14){
       cont1++;

       digitalWrite(TXIvcar, HIGH);//DATO CERO
       delayMicroseconds(700);

       digitalWrite(TXIvcar, LOW);  //UNO
       delayMicroseconds(700);
       digitalWrite(TXIvcar, HIGH);//CERO
       delayMicroseconds(700);
...
void reconnectMQTT() {
	...
	while (!client.connected()) {
      ...
      // Nos suscribimos a comandos
				char topic_comand[25];
				String topic_aux = "alarma/command/" + serie_device;
				topic_aux.toCharArray(topic_comand,25);
...
void TX_AAA(){
   ...
   digitalWrite(TXIvcar, HIGH);//DATO CERO  1
   delayMicroseconds(700);
   ...
   digitalWrite(TXIvcar, LOW);//DATO UNO 2
   delayMicroseconds(700);
   ...
   digitalWrite(TXIvcar, HIGH);//DATO CERO 3
   delayMicroseconds(700);
   ...
   digitalWrite(TXIvcar, LOW);//DATO UNO 4
   delayMicroseconds(700);
   ...
   digitalWrite(TXIvcar, HIGH);//DATO CERO 5
   delayMicroseconds(700);
   ...
   digitalWrite(TXIvcar, LOW);//DATO UNO 6
   delayMicroseconds(700);
   ...
   digitalWrite(TXIvcar, HIGH);//DATO CERO 7
   delayMicroseconds(700);
   ...
   digitalWrite(TXIvcar, LOW);//DATO UNO 8
   delayMicroseconds(700);
...
void TX_AF(){
   ...
   digitalWrite(TXIvcar, LOW);//DATO UNO 1
   delayMicroseconds(700);
   ...
   digitalWrite(TXIvcar, LOW);//DATO UNO 3
   delayMicroseconds(700);
