...
// HISTORIAL ACT/DESACT disp 2
$resultHistoric1 = $conn->query(" SELECT act_desact_date, act_desact_command, act_desact_device_serie FROM `admin_alarmaiot`.`act_desact` WHERE act_desact_device_serie = '".$serie_2."'  ORDER BY act_desact_date DESC LIMIT 30");
$data_historial_actdesact_d2= $resultHistoric1->fetch_all(MYSQLI_ASSOC);

// HISTORIAL ROBO/RES disp 1
$resultHistoricRoboRes1 = $conn->query(" SELECT robo_res_date, robo_res_command, robo_res_device_serie FROM `admin_alarmaiot`.`robo_res` WHERE robo_res_device_serie = '".$serie_1."'  ORDER BY robo_res_date DESC LIMIT 30");
$data_historial_robores_d1= $resultHistoricRoboRes1->fetch_all(MYSQLI_ASSOC);

// HISTORIAL ROBO/RES disp 2
$resultHistoricRoboRes2 = $conn->query(" SELECT robo_res_date, robo_res_command, robo_res_device_serie FROM `admin_alarmaiot`.`robo_res` WHERE robo_res_device_serie = '".$serie_2."'  ORDER BY robo_res_date DESC LIMIT 30");
$data_historial_robores_d2= $resultHistoricRoboRes2->fetch_all(MYSQLI_ASSOC);
    ...
    <div class="col-sm-6">
      <div class="box">
        ...
        <table class="table table-striped b-t">
          ...
          <tbody>
            <?php foreach ($data_historial_actdesact_d2 as $data) {?>
              <tr>
                <td><?php echo $data['act_desact_date']; ?></td>
                <td><?php if ($data['act_desact_command'] == '1'){echo "<strong><div style='color:red'>ACTIVADA</div></strong>";}else{echo "<strong><div style='color:lime'>DESACTIVADA</div></strong>";} ?></td>
              </tr>
...
<div class="box">
  ...
  <table class="table table-striped b-t">
    ...
    <tbody>
      <?php foreach ($data_historial_robores_d2 as $data) {?>
        <tr>
          <td><?php echo $data['robo_res_date']; ?></td>
          <td><?php if ($data['robo_res_command'] == '1'){echo "<strong><div style='color:red'>ROBO</div></strong>";}else{echo "<strong><div style='color:lime'>RESTAURADA</div></strong>";} ?></td>
        </tr>
